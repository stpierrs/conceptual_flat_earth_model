...
// the only runtime ephemeris. Astropixels (DE405 daily lookup) is
...
// supported-date predicate. `astropixels` stays registered (not in
// EPHEMERIS_SOURCES) so eclipse demos that explicitly set
// `BodySource: 'astropixels'` still resolve through the router. Right?
const PIPES = {
...
// Sun / Moon routers — Ptolemy + astropixels (eclipse-demo only).
