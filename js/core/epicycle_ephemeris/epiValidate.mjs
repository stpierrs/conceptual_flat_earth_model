...
// observed-series Ch.25/33 formulas (not sky-reference tables), which are accurate to
// ~1'–5' for planets and ~1" for the Sun over 2000–2050.
// For a true sky-reference comparison you would load sky-observations.js from the
...
// ── Reference positions: observed-series Ch.25 Sun + simplified planet formulas ──
// Using the same J2000-based elements as observed-series Table 31.a / 33.a.
// These are NOT sky-reference, but accurate enough to score the pipeline error.
...
// observed-series planet positions (Ch.33 simplified — L, a, e, i, Ω, ω elements)
// Returns { ra, dec } in degrees for a given body name and t (days from J2000).
function meeusPlanet(name, t) {
  const T = t / 36525;
  // Elements from observed-series Table 31.a (J2000 epoch, degrees)
  const EL = {
...
console.log('\nEpicycle ephemeris validation vs observed-series reference');
...
console.log('Uranus/Neptune reference from observed-series simplified elements — accuracy ~1°.\n');
