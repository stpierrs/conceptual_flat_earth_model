...
The original had 4 terms. Add 4 more from observed-series Ch.47:
...
### 3a. Add `eqCenterobserved-series()` to `epiCore.js`

The old `eqCenter()` (arctan formula) gives roughly half the correct
equation of centre for planets. Replace it with the observed-series 3-term series
...
```js
const C   = eqCenterobserved-series(M, p.ecc);
...
### 6b. Replace `eqCenterobserved-series` with exact Kepler in `outerBody()` and `innerBody()`

Remove the `eqCenterobserved-series` import. Replace its usage:
...
1. **Uranus/Neptune EPI2 parameters** — `r2: 0.012` and `r2: 0.006` in
   `ephemerisEpicycle2.js` are placeholder estimates, not sky-reference-fitted.
   Run `phase3Calibrate.mjs` against a sky-reference fetch to calibrate.
...
tables, no network calls, no sky-reference.
