#!/usr/bin/env node
// Parse cached AstroPixels HTML into sky-reference_reference.json
...
for (const body of BODIES) {
  ...
  for (const year of YEARS) {
    const f = `/tmp/sky-reference_cache/${body}${year}.html`;
    try {
...
console.log(`\nWrote sky-reference_reference.json  (${total} total rows, ${(JSON.stringify(result).length/1024/1024).toFixed(1)} MB)`);
