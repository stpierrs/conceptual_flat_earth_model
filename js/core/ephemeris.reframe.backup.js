...
// readouts this serial adds.
//
// S010 — the planet routine is now framed as **Tychonic geocentric**
// rather than heliocentric: the Sun has a Keplerian orbit around the
// Earth (row keyed `'sun'` in `ORBIT_EL`), and each planet has a
// Keplerian orbit around the Sun. A planet's geocentric ecliptic
// position is `sun_geo + planet_relative_to_sun`. Mathematically this
// is translation-equivalent to the prior heliocentric pipeline, so
// the numerical RA/Dec are identical — but the Earth is now the
// origin of record, and the heliocentric-chain helper `heliocentric()`
// has been replaced by the frame-neutral `keplerPosition()`. Exact
// pre-reconstruction file preserved at
...
// --- Planets (Schlyter Keplerian elements, Tychonic framing) -------------
//
// Each row: [N0, dN, i0, di, w0, dw, a0, da, e0, de, M0, dM]
//   N = longitude of ascending node (deg)
//   i = inclination to ecliptic (deg)
//   w = argument of perihelion (deg)
//   a = semi-major axis (unitless; earth-Sun distance = 1)
//   e = eccentricity
//   M = mean anomaly (deg)
// Rates are per day; values are for 1999-12-31 00:00 UT (Schlyter's
// epoch). The `a` column is in the same ratio units as Schlyter's
// original table; the actual distance scale cancels out at the
// atan2 extraction in planetEquatorial(), so no physical length
// enters the model.
//
// S010 — table renamed `ORBIT_EL` (was `PLANET_EL`) and the row
// that Schlyter labelled `'earth'` is re-keyed as `'sun'`. In the
// Tychonic geocentric framing this code now uses, that row describes
// the **Sun's Keplerian orbit around the Earth**; its elements are
// identical to Schlyter's table (Schlyter's own trick — his "earth"
// elements are numerically the Sun's geocentric orbit already). The
// other rows describe each planet's Keplerian orbit around the Sun.
const ORBIT_EL = {
  sun:     [  0,       0,            0.0000,    0,         282.9404, 4.70935e-5, 1.000000, 0,         0.016709, -1.151e-9,  356.0470, 0.9856002585],
...
// Kepler-orbit ecliptic (x, y, z) for any body whose osculating
// elements are tabulated in `ORBIT_EL`. This is a pure Kepler solver
// — it returns the body's position in the frame natural to its own
// orbit, which in the Tychonic reframing is:
//   'sun'                                 → geocentric (Sun orbits Earth)
//   'mercury', 'venus', 'mars', 'jupiter',
//   'saturn'                              → Sun-relative (planet orbits Sun)
// Unit of length is `a` (Schlyter's earth-Sun distance = 1).
//
// S010 — replaces the prior `heliocentric()` helper. The function's
// math is unchanged; only the identity / naming drops the implicit
// "frame is heliocentric" claim so the planetEquatorial() caller can
// compose the two Tychonic pieces (Sun-around-Earth + planet-around-
// Sun) without committing the module to a Sun-centred frame.
function keplerPosition(name, d) {
...
//
// S010 — Tychonic geocentric composition. Earth is the origin. The
// Sun has its own Keplerian orbit around the Earth (row `'sun'` in
// `ORBIT_EL`). Each planet has its own Keplerian orbit around the
// Sun (its own row in `ORBIT_EL`). A planet's geocentric ecliptic
// position is simply the sum of the two ellipse evaluations:
//
//     planet_geocentric  =  sun_geocentric  +  planet_relative_to_sun
//
// Mathematically this is translation-equivalent to the prior
// heliocentric pipeline — the numerical RA/Dec is identical — but
// the frame of record is now the Earth. After the ecliptic sum the
// vector is rotated into apparent-of-date equatorial coords using
// the same Schlyter obliquity `ε = 23.4393° − 3.563e-7·d`.
export function planetEquatorial(name, date) {
  const d = schlyterDay(date);
  const sunGeo       = keplerPosition('sun', d);  // Sun's orbit around Earth
  const planetRelSun = keplerPosition(name,  d);  // Planet's orbit around Sun
  const xg = sunGeo.x + planetRelSun.x;
  const yg = sunGeo.y + planetRelSun.y;
  const zg = sunGeo.z + planetRelSun.z;
...
// --- Body-position API (S009; reframed in S010) --------------------------
//
// `bodyGeocentric(name, date)` returns `{ ra, dec }` in the
// geocentric equatorial frame. This is the primary API the rest of
// the codebase uses — sky / tracker / render pipelines all consume
// this directly.
//
// `bodyHeliocentric(name, date)` returns `{ x, y, z }` — the body's
// offset FROM THE SUN in ecliptic xyz. S010 reframing: the governing
// frame of the module is Earth-centred (Tychonic); this function is
// retained for S009 tracker-API compatibility and exposes a
// geometric quantity (distance-from-Sun vector), not a frame
// coordinate. For the sun it returns the origin. For earth it
// returns `−sun_geocentric` (the geometric inverse). For planets it
// returns their Sun-relative ellipse position from `keplerPosition`.
// For the moon it returns `earth_offset_from_sun + geocentric_moon_direction`.
//
// `bodyFromHeliocentric(name, date)` composes `bodyHeliocentric` of
// the body with `bodyHeliocentric` of earth, subtracts, rotates to
// equatorial — an architectural second route that in the Tychonic
// framing reduces to the same Sun-around-Earth + Planet-around-Sun
// sum `planetEquatorial` performs directly. Output is the same
// geocentric (RA, Dec). Kept so downstream Tracker code can
// compare both routes visibly.
...
// Offset vector from the Sun to `name`, in ecliptic (x, y, z).
// Unitless (Schlyter ratio units, Earth-Sun distance ≈ 1). S010 —
// this is NOT a frame coordinate in the reconstructed module (the
// governing frame is geocentric); it's a geometric quantity kept for
// S009 API compatibility with the Tracker's diagnostic "source"
// pipeline.
export function bodyHeliocentric(name, date) {
  const d = schlyterDay(date);
  if (name === 'sun') return { x: 0, y: 0, z: 0 };
  if (name === 'earth') {
    // Earth's offset from the Sun is the geometric inverse of the
    // Sun's orbit around Earth (`ORBIT_EL.sun`).
    const s = keplerPosition('sun', d);
    return { x: -s.x, y: -s.y, z: -s.z };
  }
  if (ORBIT_EL[name] && name !== 'sun') {
    // Planet row — orbit elements are already Sun-relative in the
    // Tychonic framing, so this returns the direct Sun-to-planet
    // offset.
    return keplerPosition(name, d);
  }
  if (name === 'moon') {
