// HelioC pipeline — angular-element planet positions composed with the
// Sun's geocentric position to yield geocentric RA/Dec.
//
// **Comparison-mode only.** Default rendering pipeline is DE405
// (`ephemerisAstropixels.js`); HelioC only contributes a row in the
// side-by-side comparison HUD when `state.ShowEphemerisReadings` is
// on. With comparison off, `app.update` never invokes this pipeline.
// It also doesn't sit on the fallback chain
// (`astropixels → geocentric → ptolemy`), so a stray DE405 miss won't
// accidentally route here either.
//
// The "earth" row encodes the Sun's orbit around the Earth
// (sign pre-negated), so the geocentric planet position is:
//
//     planet_geocentric_ecliptic = planet_ecliptic + sun_geocentric
//
// Sun and Moon delegate to Meeus via `ephemerisCommon.js`
// (same as the GeoC pipeline).
...
//   a = semi-major axis (unitless ratio)
//   e = eccentricity
//   M = mean anomaly (deg)
//
// `earth` row is the pre-negated Sun-around-Earth entry (see header).
const PLANET_EL = {
...
// Days since 2000 Jan 0.0 epoch (JD 2451543.5).
...
// Ecliptic (x, y, z) in ratio units (scale cancels at atan2 in
// planetEquatorial). For the 'earth' row this returns the Sun's
// position as seen from Earth (see header).
...
// the Sun-around-Earth row with the planet's ecliptic xyz,
// then rotate ecliptic→equatorial at obliquity ε = 23.4393° − 3.563e-7·d.
export function planetEquatorial(name, date) {
  // outer planets (uranus, neptune) and Pluto aren't in
  // the element table, so return a NaN sentinel the
  ...
  const p  = heliocentric(name, d);      // planet's ecliptic position
