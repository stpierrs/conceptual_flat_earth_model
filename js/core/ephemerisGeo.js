...
// evaluated once per call and rotated into ecliptic → equatorial.
...
// Days since 2000 Jan 0.0 epoch (= 1999-12-31 00:00 UT,
...
// Solve the angular equation M = E − e·sin E for eccentric anomaly E (radians).
...
// Evaluate the Earth-focus angular ellipse at day `d`. Returns
...
// kinematic chain is: elements → mean anomaly at `d` →
// solve angle equation → orbital-plane (x, y) → ecliptic (x, y, z) →
// equatorial (RA, Dec).
export function planetEquatorial(name, date) {
  // uranus, neptune, pluto aren't in the element table;
...
// Coverage. The element table covers the 5 classical planets; sun /
// moon delegate to the Meeus pair in `ephemerisCommon.js`. Date
// span effectively unlimited within the century-scale element
// validity. Built-in corrections come from Meeus, which
